...
빔프로젝터 스크린 환경에서 **고속 LiDAR**로 비비탄 알갱이를 직접 감지하고, 실시간 적중 좌표를 유니티 게임에 전달하는 하위 시스템입니다. 초고속 UART 패킷 파싱은 백그라운드 스레드에서 처리하고, 탄환 판정 결과만 메인 스레드로 전달하는 파이프라인 구조를 따릅니다.
...
### 5.1 소프트웨어 구현 주요 모듈 (C# Scripts 구조)
